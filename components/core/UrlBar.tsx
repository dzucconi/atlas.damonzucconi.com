import { Box, Cell, CellProps, color, themeGet } from "@auspices/eos";
import { FC, useEffect, useRef, useState } from "react";
import styled from "styled-components";
import { wait } from "../../lib/wait";

type UrlBarProps = CellProps & {
  children: string;
  href: string;
  target?: string;
};

export const UrlBar: FC<UrlBarProps> = ({
  children,
  href,
  target,
  ...rest
  ...
  return (
    <Container as="a" href={href} target={target} {...rest}>
      <Hover />
      <Fade />
...
const Container = styled(Cell)<CellProps>`
  display: block;
  position: relative;
  overflow: hidden;
  border-width: 0;
